...
@MODELS.register_module()
class BaseBackboneV2(BaseModule, metaclass=ABCMeta):
    """BaseBackbone backbone used in YOLO series.
    ...
    def __init__(self,
        ...
        for idx, setting in enumerate(arch_setting):
            stage = []
            if (idx == 0):
                stage += self.build_initial_stage_layer(idx, setting)
            else:
                stage += self.build_stage_layer(idx, setting)
            if plugins is not None:
                stage += self.make_stage_plugins(plugins, idx, setting)
    ...
    @abstractmethod
    def build_initial_stage_layer(self, stage_idx: int, setting: list):
        """Build a stage layer.
